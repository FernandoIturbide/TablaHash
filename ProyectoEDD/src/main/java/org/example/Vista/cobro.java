...
//Mulriplica la cantidad de libros por su correspondiente precio, sumando a su vez todos los demás libros que se encuentren en la tabla hash (Productos)
    public void actualizarCampoTotal() {
        ToPagar = calcularPrecioTotal();
        aTextArea.setText(String.valueOf(ToPagar));
    }
    //Ayuda a que se actualice el total de la venta, cada vez que se agrega un libro a la tabla hash (Productos, a pesar de que sean dos ventanas distintas
    ...
    public void actionPerformed(ActionEvent e) {
        if (e.getSource() == cancelar) {
            ...
            System.exit(0);
            //Boton no implementado
        }
        if (e.getSource() == siguiente) {
            if (isEfectivo) {
                if (jTextField1.getText().equals("")) {
                    JOptionPane.showMessageDialog(null, "Ingrese el dinero"); // No se ingresó dinero
                } else {
                    if (Double.valueOf(jTextField1.getText()) < ToPagar) {
                        double debe = ToPagar - Double.valueOf(jTextField1.getText());
                        JOptionPane.showMessageDialog(null, "Dinero insuficiente, debe: " + debe); // No se ingresó suficiente dinero
                    } else {
                        JOptionPane.showMessageDialog(null, "Pago en efectivo realizado con éxito"); // Pago con efectivo realizado con éxito, despúes se haria el pdf del ticket
                        this.setVisible(false);

                    }
                }
            } else {
                //En caso de pagar con tarjeta
                ...
                if (numeroTarjeta.length() != 16 || !esNumero(numeroTarjeta)) {
                    ...
                    return; // Se ingresan menos o mas de 16 digitos
                }
                ...
                if (!esFechaValida(fechaExpiracion)) {
                    ...
                    return;//Solo se admiten fechas de 4 digitos
                }
                ...
                if (cvc.length() != 3 || !esNumero(cvc)) {
                    ...
                    return; //en la clave de la tarjeta solo se admiten 3 digitos o no pasará
                }
    
                //Se debería se checar con el vanco a travez de un if pero no es posible (Conocimientos insuficientes o posible fraude)
                JOptionPane.showMessageDialog(null, "Pago con tarjeta realizado con éxito");
                this.setVisible(false);
            }
        }
    }
    private boolean esNumero(String cadena) {//CVC y numero de tarjeta solo aceptan numeros
        try {
    ...
    private boolean esFechaValida(String fecha) {//Fecha de expiracion solo acepta fechas de 4 digitos
        if (fecha.length() != 4) {
            return false;
        }
        return esNumero(fecha);
    }
}
